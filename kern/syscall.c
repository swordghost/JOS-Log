...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	if(perm != (perm & PTE_SYSCALL) || (perm & (PTE_U | PTE_P)) != (PTE_P | PTE_U)){
		return -E_INVAL;
	}
	if(va != ROUNDUP(va, PGSIZE) || (int)va >= UTOP){
		return -E_INVAL;
	}
	struct Env *e;
	if(envid2env(envid, &e, 1) < 0){
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(srcva != ROUNDUP(srcva, PGSIZE) || (int)srcva >= UTOP ||
		dstva != ROUNDUP(dstva, PGSIZE) || (int)dstva >= UTOP){
		return -E_INVAL;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(srcva < (void *)UTOP && env -> env_ipc_dstva){
